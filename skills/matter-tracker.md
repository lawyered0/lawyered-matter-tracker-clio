...
| K | Client ID Verified | Text | "✓" once verified via the identity verification service; "Pending" if not yet done |
...
2026-01-22: Received draft SPA from opposing counsel (Opposing Firm LLP)
...
- **K — Client ID Verified**: Set to "✓" on file opening. ID verification is performed via **the identity verification service** (see the identity verification skill). Default to "✓" for all new matters — the user verifies ID as standard intake practice.
...
[
  ...
  {"date": "2026-04-27", "description": "Serve the required court form on defendants", "source": "Rule X — 30 days before trial"}
]
```
...
**Format**: Comma-separated names. Include role context where helpful: "David Park (co-plaintiff/witness), Sarah Kim (opposing counsel, Kim Law)".
...
6. After confirmation:
   ...
   - **Matter Folder (T)**: Search the workspace directory for a subfolder matching the client. **All matching must be case-insensitive.** First, dump the full directory listing to a text file using `ls -1 > /tmp/dirlist.txt`, then grep against that file — this avoids shell issues with special characters (colons, ampersands, parentheses, etc.) in folder names. Try matching against ALL of these permutations of the client name: "First Last" (e.g. "sam wilson"), "Last, First" (e.g. "Wilson, Sam"), "Last First" (no comma), just the last name, just the first name, and any company/entity name from the matter description or opposing party field. **Also search for the mother/father/third-party name if the matter is brought on someone else's behalf** (e.g. for "Connor v. Blake" brought by Sarah Connor on behalf of May Connor, search for "Connor", "Sarah", "Connor", and "May"). Folders are often named in lowercase or informal formats (e.g. "sam wilson" not "Wilson, Sam"), or after the entity rather than the person (e.g. "Globex Inc." not "Smith, John"), and frequently contain special characters like colons (e.g. "Sarah Connor : Connor"). Cast a wide net — grep each search term separately and case-insensitively against the text file listing. If found, write just the subfolder name exactly as it appears on disk. If not found, leave blank.
...
**Scope: NEW MATTER only.** UPDATE and CLOSE do not currently touch Clio. If the lawyer wants to change a matter's Clio record, it's done in the Clio UI directly. (May expand later if name-lookup proves reliable.)
...
- Name contains a corporate suffix (`"Inc."`, `"LLC"`, `"Ltd."`, `"Corp."`, `"Co."`) or matches a numbered-company pattern (`"1001405607 Ontario Inc."`) → **Company**. Use the full name as the company name.
...
The tool auto-sets responsible_attorney and originating_attorney to the lawyer (Clio user id YOUR_CLIO_USER_ID). Capture `id` and `display_number` from the response.
